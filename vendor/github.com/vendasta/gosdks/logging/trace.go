...
func NewClient(ctx context.Context, projectID string, opts ...option.ClientOption) (*Client, error) {
	...
	policy, err := cloudtrace.NewLimitedSampler(0, 2); if err != nil {
		return nil, err
	}
	return &Client{
		service:   apiService,
...
func (c *Client) SetSamplingPolicy(p cloudtrace.SamplingPolicy) {
	if c != nil {
		c.policy = p
	}
}
...
func traceInfoFromContext(ctx context.Context, path string) *Span {
	...
	hl, ok := md["x-cloud-trace-context"]; if !ok {
		return nil
	}
	if len(hl) != 1 {
		return nil
...
// If s is nil, does nothing and returns nil.
func (s *Span) NewRemoteChild(ctx context.Context, path string) *Span {
	if s == nil {
		return nil
	}
	newSpan := startNewChildWithContext(ctx, path, s.trace, s.span.SpanId)
	SetValue(ctx, httpHeader, newSpan.spanHeader())
	return newSpan
}
